...
def generate_image(length, row, grid, output):
    ...
    # Generate each tile of the image with a random color around the base color
    for i in range(0, x, 32):
        for j in range(0, y, 32):
            color = (
                min(max(base_color[0] + random.randint(-30, 30), 0), 255),
                min(max(base_color[1] + random.randint(-30, 30), 0), 255),
                min(max(base_color[2] + random.randint(-30, 30), 0), 255)
            )
